...
def get_user_from_token(request):
    ...
    if decoded_payload is not None:
        ...
        ).first()  # Check if decoded_payload is not None
        if user:
            return user
    return None
...
def decode_jwt(token):
    try:
        decoded = jwt.decode(
            token, api_settings.SIGNING_KEY, algorithms=[api_settings.ALGORITHM]
        )
        return decoded
...
def get_auth_url():
    ...
    if not OAUTH_CLIENT_ID or not OAUTH_REDIRECT_URI:
        return (
            None,
            "Missing OAUTH_CLIENT_ID or OAUTH_REDIRECT_URI in environment variables",
        )
...
def handle_user_oauth(user_data):
    ...
    if user is None:
        user = User(
            username=user_data.get("login"), email=user_data.get("email"), is_oauth=True
        )
        ...
        if image_url:
            ...
            if response.status_code == 200:
                user.avatar.save(
                    f"avatar_{user.username}.jpg", ContentFile(response.content)
                )
        user.save()
